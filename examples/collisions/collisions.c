...
int main(int argc, char **argv) {
    ...
    InitWindow(screenWidthInPixels, screenHeightInPixels, "raytmx collisions example");
    SetTargetFPS(60);
    ...
    Vector2 mapCenter;
    mapCenter.x = (float)(map->width * map->tileWidth) / 2.0f;
    mapCenter.y = (float)(map->height * map->tileHeight) / 2.0f;
            ...
            for (size_t j = 0; j < group.objectsLength; j++) {
                ...
                for (size_t k = 0; k < object.propertiesLength; k++) {
                    ...
                    if (strcmp(property.name, "name") == 0 && property.type == PROPERTY_TYPE_STRING &&
                        ...
                        polygonCenter.x = (float)(object.x + (object.width / 2.0));
                        polygonCenter.y = (float)(object.y + (object.height / 2.0));
